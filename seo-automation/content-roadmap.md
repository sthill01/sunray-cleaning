# Content Roadmap

Today is April 25, 2026. The original April 20, April 22, and April 24 publish dates in the source calendar are now in the past, so this roadmap starts with the next actionable content cycle.

## Immediate Webflow Priorities

1. Review and publish the three completed CMS drafts:
   - The Complete Guide to Airbnb & VRBO Cleaning in Park City (2026)
   - Post-Ski-Season Deep Clean Checklist for Park City Rental Owners
   - Red Ledges Home Cleaning: A Guide for Luxury Heber Homeowners
2. Fix the recurring cleaning H1 typo in Webflow Designer:
   - Current typo: `Reccuring Cleaning for your Home`
   - Correct: `Recurring Cleaning for Your Home`
3. Add the seven 301 redirects listed in the source strategy doc.
4. Delete or unpublish the duplicate Webflow pages:
   - `/untitled`
   - `/service-location/salt-lake-county-copy`

## Next Four Webflow Drafts

| Target Date | Type | Draft |
|---|---|---|
| April 27, 2026 | Commercial | Heber City Move-In/Move-Out Cleaning: What New Residents Should Know |
| April 29, 2026 | Educational | Getting Your Park City Home Ready for Summer Guests |
| May 1, 2026 | Hyper-local | Park City Luxury Home Cleaning: Deer Valley Standards Explained |
| May 4, 2026 | Commercial | Jordanelle Vacation Rental Turnover: What Lakeside Owners Need |

## Highest-ROI Page Types

- Pricing pages: house cleaning cost, deep clean cost, Airbnb cleaning cost.
- Short-term rental pages: Airbnb/VRBO, Canyons Village, Old Town, Deer Valley, Jordanelle.
- Luxury neighborhood pages: Red Ledges, Deer Valley, Promontory, Park Meadows.
- Seasonal pages: post-ski clean, summer guest prep, pre-ski season reset, Sundance turnover.
- Comparison pages: deep vs regular cleaning, move-in vs move-out, linen service vs on-site laundry.

## LLM + Local SEO cluster pages (drafts staged in repo)

These are Webflow-ready drafts created to target AI-search prompts and high-conversion local queries:

- Park City STR landing page draft: `webflow-content-drafts/2026-05-15-park-city-airbnb-vrbo-turnover-cleaning.md`
- Park City “what’s included” blog draft: `webflow-content-drafts/2026-05-15-what-is-included-turnover-cleaning-park-city.md`
- Park City Airbnb cleaning cost blog draft: `webflow-content-drafts/2026-05-15-airbnb-cleaning-cost-park-city.md`
- Park City deep-clean cost blog draft: `webflow-content-drafts/2026-05-15-deep-cleaning-cost-park-city.md`
- Summit County hub page draft: `webflow-content-drafts/2026-05-15-summit-county-house-cleaning.md`
- Wasatch County hub page draft: `webflow-content-drafts/2026-05-15-wasatch-county-house-cleaning.md`
- Red Ledges location page draft: `webflow-content-drafts/2026-05-15-service-location-red-ledges.md`
- Deer Valley location page draft: `webflow-content-drafts/2026-05-15-service-location-deer-valley.md`
- Property manager vendor page draft: `webflow-content-drafts/2026-05-15-preferred-vendor-property-managers.md`
- Move-in/move-out checklist blog draft: `webflow-content-drafts/2026-05-15-move-in-move-out-cleaning-checklist-wasatch-back.md`
